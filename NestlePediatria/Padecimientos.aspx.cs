...
namespace NestlePediatria
{
    public partial class Padecimientos : System.Web.UI.Page
    {
        ...
        protected void dgPadecimientos_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "btnEditar")
            {
                try
                {
                    ...
                    Response.Redirect("AddEditPadecimiento.aspx?idPadecimiento=" + id);
            ...
            if (e.CommandName == "btnEliminar")
            {
                try
                {
                    ...
                    PadecimientoDAO bdJugador = new PadecimientoDAO();

                    if (bdJugador.EliminarPadecimiento(indice))
                    {
        ...
        protected void dgPadecimientos_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        {
            Response.Redirect("AddEditPadecimiento.aspx");
        }

        protected void CargaPadecimientos()
        {
            List<Padecimiento> padecimientoGrid = PadecimientoDAO.Consulta();
            dgPadecimientos.DataSource = padecimientoGrid;
            dgPadecimientos.DataBind();
