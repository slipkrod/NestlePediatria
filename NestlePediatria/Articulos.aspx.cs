...
namespace NestlePediatria
{
    public partial class Articulos : System.Web.UI.Page
    {
        ...
        protected void dgArticulos_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "btnEditar")
            {
                try
                {
                    ...
                    Response.Redirect("AddEditArticulo.aspx?idArticulo=" + id);
            ...
            if (e.CommandName == "btnEliminar")
            {
                try
                {
                    ...
                    if (bdArticulo.EliminarArticulo(indice))
                    {
        ...
        protected void dgArticulos_RowDataBound(object sender, GridViewRowEventArgs e)
        {
        ...
        {
            Response.Redirect("AddEditArticulo.aspx");
        }


        protected void CargaArticulos()
        {
            List<Articulo> articuloGrid = ArticuloDAO.Consulta(int.Parse(Session["id"].ToString()), int.Parse(Session["rol"].ToString()));
            dgArticulos.DataSource = articuloGrid;
